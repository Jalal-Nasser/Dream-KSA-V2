...
export default function RoomChat() {
  ...
  React.useEffect(() => {
    ...
    return () => { mounted = false; supabase.removeChannel(channel); channelRef.current = null; };
  }, [id]);
  ...
  const send = async () => {
    ...
    const msg: Msg = {
      ...
      from: user?.email || user?.id?.slice(0, 6) || 'أنا',
      text: text.trim(),
      at: Date.now()
    };
    if (!msg.text) return;
    ...
    // optional persist
    supabase.from('messages').insert({ room_id: id, user_id: user?.id, content: msg.text }).then(() => {}).catch(() => {});
  };
